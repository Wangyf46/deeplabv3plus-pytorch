...
from experiment.exp_voc.config import cfg
...
class deeplabv3plus(nn.Module):
	def __init__(self, cfg):
		...
		input_channel = 2048		
		self.aspp = ASPP(dim_in=input_channel,  # 2048
				dim_out=cfg.MODEL_ASPP_OUTDIM,  # 256
				rate=16//cfg.MODEL_OUTPUT_STRIDE, # 1
	...
	def forward(self, x):
		# ipdb.set_trace()
		# (n, 3, 512,512)
		x_bottom = self.backbone(x)  #(n, 2048, h, w)
